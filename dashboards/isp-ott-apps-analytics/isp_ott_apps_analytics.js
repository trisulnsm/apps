...
class ISPOverviewMapping{
  ...
  async add_form(opts){
    ...
    var js_params = {meter_details:all_dropdown,
      selected_cg : selected_router || localStorage.getItem("apps.ott_appsanalytics.last-selected-router"),
      selected_st : selected_interface || localStorage.getItem("apps.ott_appsanalytics.last-selected-interface"),
  ...
  submit_form(){
    ...
    localStorage.setItem("apps.ott_appsanalytics.last-selected-router", 
                            $('.ott_appsanalytics_form select[name="routers"] option:selected').val());

    localStorage.setItem("apps.ott_appsanalytics.last-selected-interface", 
                            $('.ott_appsanalytics_form select[name="interfaces"] option:selected').val());
  ...
  async draw_table(){
    ...
    let rows = [];
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".toppers_table_div").find('.animated-background').remove();
    var table = this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".toppers_table").find("table");
  ...
  async draw_chart(){
    ...
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".donut_chart").append($("<div>",{id:this.dount_div_id}));
    this.trfchart_div_id = `traffic_chart_${this.meter_index}_${this.rand_id}`;
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".traffic_chart").append($("<div>",{id:this.trfchart_div_id}));
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".traffic_chart_div").find(".animated-background").remove();
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".donut_chart_div").find(".animated-background").remove();
  ...
  async draw_sankey_chart(){
    ...
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".sankey_chart").append($("<div>",{id:this.sankey_div_id}));
    ...
    this.data_dom.find(`#ott_apps_overview_${this.meter_index}`).find(".sankey_chart_div").find(".animated-background").remove();
      ...
      case 2:
       ...
       window.open("/newdash/index?" + 
                    $.param({
                        ...
                        window_fromts:this.tmint.from.tv_sec,
                        window_tots:this.tmint.to.tv_sec,
                        "dash_key_regex":"gitott_appsAnalyticsDrilldown"
                    }));
        break;
