...
var EdgeVertexMonitor= $.klass({
  ...
  add_form:function(){
    ...
    if(localStorage.getItem("/trisul/edge_vertex/monitor/valid_input")==1)
    {
      ...
      default_meters   = localStorage.getItem("/trisul/edge_vertex/monitor/cgguid");
      ...
      to_date= localStorage.getItem("/trisul/edge_vertex/monitor/top_date")
 ...
 //UI
  reset_ui:function(){
    ...
    panel.find('.panel-heading').html('Usage report')
  ...
  get_resp:function(req){
    var cthis = this;
    var meter_type = cthis.all_meters_type[this.cgguid];
    return  get_response(req,function(resp){
      var o = {readable:resp.key.readable,label:resp.key.label};
      var v = [];
...
function run(opts){
  ...
  $(document).ajaxStop(function () {
    if(evm.form !=undefined && evm.auto_submitted!=true && opts["valid_input"]==1){
      evm.form.submit();
