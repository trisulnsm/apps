  ...
  update_description(){
    ...
    if (readable.length > 1){
      description = `${description}  interface ${label[1]} -> Prefix ${readable[0]} `
    }else{
  ...
  submit_form(){
    this.mk_time_interval();
    var newasn = this.form.find("#drilldown_prefix").val();
  ...
  draw_table(){
    var table = this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(".toppers_table").find("table");
    this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(".toppers_table").removeClass('animated-background');
  ...
  async draw_dount_chart(){
    this.dount_div_id = `prefix_drilldown_${this.meter_name}_dount`;
    this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(".donut_chart").removeClass('animated-background');
    this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(".donut_chart").append($("<div>",{id:this.dount_div_id}));
    let cgtoppers =  this.cgtoppers_resp.keys.slice(0,this.maxitems);
    var values = [];
    var labels = [];
    for(let i= 0 ; i <  cgtoppers.length  ; i++){
  ...
  async draw_traffic_chart(){
    ...
    this.traf_chart_id = `prefix_drilldown_${this.meter_name}_traffic_chart`
    this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(`.interfaces_traffic_chart`).attr("id",this.traf_chart_id);
  ...
  async draw_sankey_chart(filter_cgbase){
    ...
    this.sankey_div_id = `prefix_drilldown_${this.meter_name}_sankey`;
    this.data_dom.find(`#prefix_drilldown_${this.meter_name}`).find(".interfaces_sankey_chart").append($("<div>",{id:this.sankey_div_id}));
