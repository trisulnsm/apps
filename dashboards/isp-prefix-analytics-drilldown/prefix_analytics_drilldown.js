...
class ISPPrefixDrilldownMapping{
  ...
  submit_form(){
    ...
    this.key=this.form.find('#drilldown_prefix').val();
    if(this.key.length ==0 ){
      alert("Prefix filed can't be empty.");
  ...
  async get_keyt(){
    let resp=await fetch_trp(TRP.Message.Command.SEARCH_KEYS_REQUEST, {
      counter_group: "{2BE2A3B6-613D-4216-0737-3684E824EA33}",
      label:this.key,
    });
    ...
    for(let meter in this.meters){
      ...
      nodes.push({find_by:`#prefix_drilldown_${idx}_donut`,type:"svg",header_text:"auto",h1:"h5",float:"right"});
      nodes.push({find_by:`#prefix_drilldown_${idx}_traffic_chart`,type:"svg",header_text:"auto",h1:"h5",float:"right"});
      nodes.push({type:"page_break"});
      nodes.push({find_by:`#prefix_drilldown_${idx}_sankey`,type:"svg",header_text:"auto",h1:"h5",float:"right"});
    ...
    new ExportToPDF({add_button_to:".add_download_btn",
                      ...
                      download_file_name:"prefix_analytics_drilldown",
                      report_opts:{
  ...
  async draw_toppers_table(meter_name,idx){
    ...
    var table = this.dom.find(`#prefix_drilldown_${idx}`).find(".toppers_table").find("table");
    table.attr("id",`table_${idx}`);
    this.dom.find(`#prefix_drilldown_${idx}`).find(".toppers_table_div").find('.animated-background').remove();
  ...
  async draw_donut_chart(meter_name,idx){
    this.donut_div_id = `prefix_drilldown_${idx}_donut`;
    this.dom.find(`#prefix_drilldown_${idx}`).find(".donut_chart_div").find('.animated-background').remove();
    this.dom.find(`#prefix_drilldown_${idx}`).find(".donut_chart").append($("<div>",{id:this.donut_div_id}));
  ...
  async draw_traffic_chart(meter_name,idx){
   ...
   async draw_sankey_chart(meter_name,midx){
    ...
    this.sankey_div_id = `prefix_drilldown_${midx}_sankey`;
    this.dom.find(`#prefix_drilldown_${midx}`).find(".interfaces_sankey_chart").append($("<div>",{id:this.sankey_div_id}));
    this.dom.find(`#prefix_drilldown_${midx}`).find('.sankey_chart_div').find(".animated-background").remove();
    // Get Bytes Toppers
  ...
  update_description(){
    let description = "Drilldown for Prefix"
...
//# sourceURL=prefix_drilldown_mappings.js
