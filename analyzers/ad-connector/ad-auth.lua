...
TrisulPlugin = {
    ...
    onload = function() 
		...
		T.regex_kerb_logon  = T.re2('^<\\d+>([A-Za-z]{3} [0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}).+An account was successfully logged on.+?Account Name:\\s+(\\w+).+?Account Domain:\\s+(\\w+).+?Workstation Name: (\\S+).+?Source Network Address: (\\d{1,3}(?:\\.\\d{1,3}){3})')
