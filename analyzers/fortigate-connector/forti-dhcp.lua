...
TrisulPlugin = { 
  ...
  onload = function()
	...
	T.re2_Fortigate_Forward=T.re2('timestamp=(\\d+).*srcip="(\\S+)".*srcname="(\\S+).*user="(\\S+)".*app="(\\S+)".*sentbyte=(\\d+).*rcvdbyte=(\\d+)')
  ...
  simplecounter = {
    ...
    onpacket = function(engine,layer)
	  ...
	  bret, starttm,srcip,srcname,userid,app,sentbyte,recvbyte= T.re2_Fortigate_Forward:partial_match_n(syslogstr)
	  if bret  then 
